...
`structuredSCA()` is an algorithm for sparse SCA - that is, a Lasso penalty and a Group Lasso penalty are added to the SCA model. The algorithm can itentify common and distinctive components as long as the proper tuning parameters for Lasso and Group Lasso are chosen. Note that this algorithm uses a MM procedure, meaning that the estimation takes some time to converge. Uers may use `cv_sparseSCA()` to find suitable values for the Lasso and Group Lasso tuning parameters. `maxLGlasso()` helps to identy the max values for the Lasso and Group Lasso tuning parameters. Also noted that, `structuredSCA()` incorporates a multi-start procedure to deal with the local minima problem. For technical details, see
