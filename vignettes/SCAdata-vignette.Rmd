...
### An example
Here we use the dataset `RnaDna` included in this pacakge to illustrate how the functions are used. Here we will perform a joint analysis on the DNA data and RNA data. Notice that the both datasets have the same subjects (44 rows).
```{r}
data("RnaDna")
DNA_data <- RnaDna[[1]]
dim(DNA_data)
sum(is.na(DNA_data))  # thus, there is no missing data
RNA_data <- RnaDna[[2]]
dim(RNA_data)
sum(is.na(RNA_data))  # thus, there is no missing data
```
## Data pre-processing
DNA data and RNA data need to be pre-processed.
```{r}
DNA_stand <- mySTD(DNA_data)
RNA_stand <- mySTD(RNA_data)
```
